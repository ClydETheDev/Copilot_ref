...
async function load(client: Discord.Client, cm: Manager) {
	cm.register({
		...
		handler: async msg => {
			...
			const user = await getUser(msg, args[1]);
			if (!user)
				return msg.reply("0: usage is `f!warn <mention> [reason]`!");
			const member = await msg.guild?.members.fetch(user.id);
			if (!member)
				return msg.reply(
					"0: this is either unexcepted or member is not in this server."
				);

			let lvl = 0;
			if (member.roles.cache.has(roles[0])) lvl = 1;
			if (member.roles.cache.has(roles[1])) lvl = 2;
			if (member.roles.cache.has(roles[2]))
				return msg.reply("User is already in thin ice!");
			if (lvl == 0) {
				member.roles.add(roles[0]);
				msg.reply(
					`Warned user \`${member.user.tag}\`${
						args[2] ? ` with the reason ${args[2]}` : ""
			...
			if (logChannel?.isTextBased())
				logChannel.send(
					`\`${msg.author.tag}\` (${msg.author.id}) warned \`${
						member.user.tag
					}\`${
						args[2]
							? ` with the reason ${args[2]}`
							: " with no reason"
					}! this is their warn #${lvl + 1}`
				);
		}
	});
}
